...
class NexusFileManager(FileManager):
    ...
    def write_file(output_path: str, output_data: List[Dict[str, str]]):
        number_taxa, number_characters, symbols = NexusFileManager.extract_info_to_header(output_data)
        with open(output_path, mode='w', encoding='utf-8') as nexus_file:
            first_line = '#NEXUS\n\n'
            begin_data = 'BEGIN DATA;\n'
            header_first_line = f'  DIMENSIONS NTAX={number_taxa} NCHAR={number_characters}\n'
            header_second_line = f' FORMAT SYMBOLS="{symbols}" MISSING=N GAP=- interleave=no;\n'
    ...
    def extract_info_to_header(output_data: List[Dict[str, str]]):
            ...
            for item in extracted_symbols:
                ...
                symbols += symbol
            header_info = (number_taxa, number_characters, symbols)
            return header_info